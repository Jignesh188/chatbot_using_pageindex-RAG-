...
def count_tokens(text, model=None):
    ...
    try:
        ...
        # Fallback to a standard tokenizer for non-OpenAI models (like Ollama)
        enc = tiktoken.get_encoding("cl100k_base")
    tokens = enc.encode(text)
    return len(tokens)

def Ollama_API_with_finish_reason(model, prompt, api_key=OLLAMA_API_KEY, chat_history=None):
    max_retries = 10
    client = openai.OpenAI(base_url="http://localhost:11434/v1", api_key=api_key if api_key else "ollama")
...
def Ollama_API(model, prompt, api_key=OLLAMA_API_KEY, chat_history=None):
    max_retries = 10
    client = openai.OpenAI(base_url="http://localhost:11434/v1", api_key=api_key if api_key else "ollama")
    for i in range(max_retries):
        try:
...
def get_page_tokens(pdf_path, model="gpt-oss:120b-cloud", pdf_parser="PyPDF2"):
    try:
        enc = tiktoken.encoding_for_model(model)
    except KeyError:
        enc = tiktoken.get_encoding("cl100k_base")
...
def check_token_limit(structure, limit=110000):
    ...
    for node in list:
        num_tokens = count_tokens(node['text'], model='gpt-oss:120b-cloud')
        if num_tokens > limit:
